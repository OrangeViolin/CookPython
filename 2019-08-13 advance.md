...
比如注明的裴波那契数列，我们的任务是简单输出裴波那契数列的前 N 个数：
...
但是，如果直接是 print 那么输出的是一个一个的数字，其他函数无法获得该函数生成的数列。

要提高 fab 函数的可复用性，最好不要直接打印出数列，而是返回一个 List，把fib函数变成generator，只需要把print(b)改为yield b就可以了。
